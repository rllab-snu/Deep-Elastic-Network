        ...
        if first_block:
            ...
            for i in range(lv):
                ...
                with tf.variable_scope('hidden_lv%d' % i):
                    ...
                    conv1_ = tf.sign(tf.nn.relu(keep - i)) * tf.nn.conv2d(input_layer, filter=filter, strides=[1, 1, 1, 1], padding='SAME')
                    conv_list.append(conv1_)
            ...
            for i in range(lv):
                if i is 0:
                    ...
                    r = int(output_channel / pow(2, lv - i))
                with tf.variable_scope('hidden_lv%d' % i):
                    conv1_ = bn_relu_conv_layer(input_layer, [3, 3, input_channel, r], stride)
                    conv_list.append(conv1_)
            conv1 = tf.concat(conv_list, 3)
    ...
    with tf.variable_scope('resnet', reuse=reuse):
        for i in range(n):
            with tf.variable_scope('conv1_%d' % i, reuse=reuse):
                if i == 0:
                    conv1 = residual_block(layers[-1], 64, keeps[:, j, :, :, :], first_block=True)
                else:
                    conv1 = residual_block(layers[-1], 64, keeps[:, j, :, :, :])
                j += 1
                layers.append(conv1)

        for i in range(n):
            with tf.variable_scope('conv2_%d' % i, reuse=reuse):
                conv2 = residual_block(layers[-1], 128, keeps[:, j, :, :, :])
                j += 1
                layers.append(conv2)
